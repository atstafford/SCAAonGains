...
for (i in 1:nrow(hg38.length)) {
  output[[i]] <- paste(i, sample(1:hg38.length$length[i], 10000), sep=":")
}
...
# find radnom that arnet near BP
drop <- list()
for (i in 1:length(randomPoints)) {
  print(paste(i,"/",length(randomPoints)))
  chr <- paste("chr", sub("\\:.*", "", randomPoints[i]), sep="")
  point <- as.numeric(sub(".*:", "", randomPoints[i])) 
  wd <- breakpoints_dWGS[which(breakpoints_dWGS$chr==chr),]
  for (j in 1:nrow(wd)) {
    if( abs(point-wd$breakpoint[j])<5000000  ) {
      drop[[i]] <- TRUE
      break
    } else {
      drop[[i]] <- FALSE
...
sum(drop==TRUE)
sum(drop==FALSE)
randomPoints_5Mbaway <- randomPoints[drop==FALSE]
...
for (i in 1:nrow(breakpoints_dWGS)) {
  ...
  chr <- breakpoints_dWGS$chr[i]
  point <- breakpoints_dWGS$breakpoint[i]
  wd <- gap[which(gap$chrom==chr),]
  for (j in 1:nrow(wd)) {
    if( between(point, wd$chromStart[j], wd$chromEnd[j])  ) {
      drop[[i]] <- TRUE
...
sum(drop==TRUE)
sum(drop==FALSE)
breakpoints_dWGS$gap <- unlist(drop)

# remove outllier patients and clonality cutoff. remove dups ----

breakpoints_dWGS_sub <- breakpoints_dWGS[which(breakpoints_dWGS$patient %!in% c("C516","C542","C543")),]
breakpoints_dWGS_sub <- breakpoints_dWGS_sub[which(breakpoints_dWGS_sub$BPclonality>0.5),] 
breakpoints_dWGS_sub <- breakpoints_dWGS_sub[,-c(1,7)] # wrong? remove border col
breakpoints_dWGS_sub <- breakpoints_dWGS_sub[!duplicated(breakpoints_dWGS_sub),]
