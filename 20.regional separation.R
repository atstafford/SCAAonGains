...
mat <- assay(deseqVst.dWGS.cohort)
dist <- dist(t(mat), method = 'euclidean')
clusters <- hclust(dist, method = "complete")
plot(clusters)
...
pca_plot.perpatient <- ggplot(df, aes(x=PC1, y=PC2)) + 
...
top_loadings <- loadings %>% 
  filter(peak %in% top_peaks)

loadings_plot <- ggplot(data = top_loadings) +
  geom_segment(aes(x = 0, y = 0, xend = PC1, yend = PC2), 
               arrow = arrow(length = unit(0.1, "in")),
               colour = "brown") +
  geom_text_repel(aes(x = PC1, y = PC2, label = peak),
            nudge_y = 0.005, size = 3) +
  scale_x_continuous(expand = c(0.02, 0.02))
